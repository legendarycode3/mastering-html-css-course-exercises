...
1. Element Selector **""**: Selects all instances of an element, such as p for all paragraphs. The CSS element selector selects all elements with the specified element name.The element selector uses the HTML tag name itself as the selector, like, h2 {color: blue;
  text-align: center;} <br/>
   ...
   elements by their node name in the HTML document.  </br>
...
flex or inline-flex. This makes all of the container's direct children into flex items,
inline-flex. This makes all of the container's direct children into flex items, whose layout, alignment, and spacing can then be controlled using various flexbox properties.  </br>
...
1iii. align-items: Aligns flex items along the cross axis (e.g., flex-start, flex-end, center, baseline, stretch). Aligns flex items along the cross axis (perpendicular to the main axis). <br/>
